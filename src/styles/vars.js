...
export default {
  ...
  borderColor: color(base.light).darken(0.06).string(),
  cardColor: '#fff',

  backgroundLine: color(base.light).darken(0.03).string(),

  sidebarBg: base.dark,
  sidebarBgActive: color(base.dark).darken(0.2).string(),
  sidebarColor: base.light,
  sidebarBorder: color(base.light).alpha(0.15).string(),
  sidebarShadow: color(base.dark).darken(0.4).string(),

  // Size variables
  contentWidth: 100,

  // Something like SASS functions
  border: (borderColor = color(base.light).darken(0.06).string()) => ({
